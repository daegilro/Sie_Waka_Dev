...
{
    this->_temperature    = 19.0;
    this->_phValue        = 7.0;
    this->_acidVoltage    = 2383;    //buffer solution 4.0 at 25C
    this->_neutralVoltage = 1775;     //buffer solution 7.0 at 25C
    this->_voltage        = 1775;
}
...
void DFRobot_PH::begin()
{
    ...
    if(EEPROM.read(PHVALUEADDR)==0xFF && EEPROM.read(PHVALUEADDR+1)==0xFF && EEPROM.read(PHVALUEADDR+2)==0xFF && EEPROM.read(PHVALUEADDR+3)==0xFF){
        this->_neutralVoltage = 1775;  // new EEPROM, write typical voltage
    ...
    if(EEPROM.read(PHVALUEADDR+4)==0xFF && EEPROM.read(PHVALUEADDR+5)==0xFF && EEPROM.read(PHVALUEADDR+6)==0xFF && EEPROM.read(PHVALUEADDR+7)==0xFF){
        this->_acidVoltage = 2383;  // new EEPROM, write typical voltage
...
{
    float slope = (7.0-4.0)/((this->_neutralVoltage-1500)/3.0 - (this->_acidVoltage-1500)/3.0);  // two point: (_neutralVoltage,7.0),(_acidVoltage,4.0)
    float intercept =  7.0 - slope*(this->_neutralVoltage-1500)/3.0;
    //Serial.print("slope:");
    //Serial.print(slope);
    //Serial.print(",intercept:");
    //Serial.println(intercept);
    this->_phValue = slope*(voltage-1500)/3.0+intercept;  //y = k*x + b
    return _phValue;
}
...
void DFRobot_PH::phCalibration(byte mode)
{
    ...
    switch(mode){
        ...
        case 2:
        if(enterCalibrationFlag){
            if((this->_voltage>1500)&&(this->_voltage<2000)){        // buffer solution:7.0{
                Serial.println();
            ...
            }else if((this->_voltage>2050)&&(this->_voltage<2700)){  //buffer solution:4.0
                Serial.println();
        ...
        if(enterCalibrationFlag){
            ...
            if(phCalibrationFinish){
                if((this->_voltage>1500)&&(this->_voltage<2000)){
                    EEPROM_write(PHVALUEADDR, this->_neutralVoltage);
                }else if((this->_voltage>2050)&&(this->_voltage<2700)){
                    EEPROM_write(PHVALUEADDR+4, this->_acidVoltage);
                }
